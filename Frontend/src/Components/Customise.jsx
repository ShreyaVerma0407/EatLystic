...
        if (!userId) {
            setFeedbackMessage({ type: 'warning', text: 'You must be logged in to view and save recipes.' });
            return;
        }
        setIsLoading(true);
        try {
...
        e.preventDefault();
        if (!userId) {
            setFeedbackMessage({ type: 'danger', text: "Error: User not authenticated." });
            return;
        }
        setIsLoading(true);
        setFeedbackMessage({ type: '', text: '' });
...
    return (
        <div className="bg-dark text-white font-sans py-5" style={{ minHeight: '100vh', paddingTop: '80px' }}>
            <Navbar /> {/* ✅ Using your Navbar component here */}
            <div className="container mt-5">
                <h1 className="text-center mb-4">Customise Recipes</h1>
                {!userId && (
                    <div className="alert alert-info text-center" role="alert">
                        Please log in to create and save your own recipes.
                    </div>
                )}
                {userId && (
...
                    </div>
                )}
                {isFormVisible && userId && (
                    <div className="card bg-secondary text-white p-4 mb-5">
                        <h2 className="card-title text-center mb-4">{editingRecipeId ? 'Edit Recipe' : 'Create New Recipe'}</h2>
...
                                <button type="submit" className="btn btn-lg" style={{ backgroundColor: '#ff69b4', borderColor: '#ff69b4', color: 'white' }} disabled={isLoading}>
...
                                        <div className="card bg-secondary text-white w-100 d-flex flex-column">
