...
def match_score(query: str, product_title: str) -> float:
    ...
    # Weighted composite
    composite = (token_set * 0.5) + (partial * 0.3) + (token_sort * 0.2)
    return round(composite, 2)


def filter_relevant(query: str, products: list[dict], threshold: float = RELEVANCE_THRESHOLD) -> list[dict]:
    """
...
def best_match(query: str, products: list[dict]) -> dict | None:
    ...
    result  = process.extractOne(query, titles, scorer=fuzz.token_set_ratio)

    if result:
        best_title, score, idx = result
        if score >= RELEVANCE_THRESHOLD:
            return {**products[idx], "match_score": score}
